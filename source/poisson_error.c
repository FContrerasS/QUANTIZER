...
static bool comparison_with_previous_solution(const struct node *ptr_node)
{
  ...
  vtype threshold_minus = 1.0 - _ERROR_THRESHOLD_IN_THE_POISSON_EQUATION_2;
  vtype threshold_plus = 1.0 + _ERROR_THRESHOLD_IN_THE_POISSON_EQUATION_2;
...
static bool poisson_error_mehod_0(const struct node *ptr_node)
{
  ...
  vtype rhomean_times_4piG = 4.0 * _G_ * _PI_ * ptr_node->node_mass / (size * H * H * H);
...
static bool poisson_error_mehod_2(const struct node *ptr_node)
{
  ...
  error = error / (mycbrt(ptr_node->grid_intr_size) * mycbrt(ptr_node->grid_intr_size));
...
static bool conj_grad_error(struct node *ptr_node, vtype *error, vtype error_total_pow2)
{
  ...
  vtype rhomean_times_4piG = 4.0 * _G_ * _PI_ * ptr_node->node_mass / (ptr_node->cell_size * H * H * H);
  ...
  {
    vtype aux_tol = ptr_node->grid_intr_size * (_ERROR_THRESHOLD_IN_THE_POISSON_EQUATION_ * rhomean_times_4piG) * (_ERROR_THRESHOLD_IN_THE_POISSON_EQUATION_ * rhomean_times_4piG) * (mycbrt(ptr_node->grid_intr_size) * mycbrt(ptr_node->grid_intr_size));
